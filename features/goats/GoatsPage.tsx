...
import { PawPrint, Trash2, Plus, Utensils, HeartPulse, Milk, Scale, Baby } from 'lucide-react';
...
type GoatsTab = 'overview' | 'herd' | 'kidding' | 'milk' | 'weights' | 'health' | 'feed';

const eventTypes: HerdEventType[] = ['vaccination', 'treatment', 'deworming', 'sold', 'dead', 'other'];

export function GoatsPage() {
  const { profile } = useAuth();
  const { orgId, herd, kidding, milkLogs, weights, events, feedLogs, loading, errors } = useGoatData();
  ...
  const [tab, setTab] = useState<GoatsTab>('overview');
  const [notice, setNotice] = useState<{ tone: 'success' | 'error'; text: string } | null>(null);
  const [busy, setBusy] = useState(false);

  const emptyAnimal = {
    tag: '', name: '', type: 'meat' as GoatType, sex: 'doe' as GoatSex,
  ...
  const [editingKidding, setEditingKidding] = useState<KiddingLog | null>(null);
  ...
  const does = activeHerd.filter(a => a.sex === 'doe');
  const bucks = activeHerd.filter(a => a.sex === 'buck');
  const milkDoes = does.filter(a => a.type === 'dairy' || a.type === 'dual');
  ...
  const stats = useMemo(() => {
    ...
    const kidsBorn = kidding.reduce((s, k) => s + (k.born || 0), 0);
    return { active: activeHerd.length, milkThisMonth, kidsBorn };
  }, [activeHerd.length, milkLogs, kidding]);
  ...
  function startEditAnimal(animal: Goat) {
    setEditingAnimalId(animal.id);
    setAnimalForm({
      tag: animal.tag || '',
      name: animal.name || '',
      type: animal.type || 'meat',
      sex: animal.sex || 'doe',
      breed: animal.breed || '',
      dob: animal.dob || '',
  ...
  async function submitAnimal(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (herd.find(a => a.tag === tag && a.id !== editingAnimalId)) return flash('error', 'Another animal already uses this tag.');
    ...
    try {
      ...
      if (editingAnimalId) {
        await setRecord(orgId, collectionNames.goats, editingAnimalId, payload, uid);
        flash('success', `${tag} updated.`);
      } else {
        const ref = await addRecord(orgId, collectionNames.goats, { ...payload, status: 'active' }, uid);
        const initial = Number(animalForm.initialWeight) || 0;
        if (initial > 0) {
          await addRecord(orgId, collectionNames.goatWeights, { animalId: ref.id, date: animalForm.dob || today(), weight: initial, notes: 'Initial weight' }, uid);
        }
  ...
  async function submitKidding(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!canWrite) return;
    const formElement = event.currentTarget;
    const form = new FormData(formElement);
    const doeId = asString(form.get('doeId'));
    if (!doeId) return flash('error', 'Select a doe.');
    const born = asNumber(form.get('born'));
    const record = {
      doeId,
      buckId: asString(form.get('buckId')) || '',
      date: asString(form.get('date')) || today(),
      born,
      alive: asNumber(form.get('alive')) || born,
      gestation: asNumber(form.get('gestation')) || 0,
      notes: asString(form.get('notes'))
    };
    setBusy(true);
    try {
      if (editingKidding) {
        await setRecord(orgId, collectionNames.goatKidding, editingKidding.id, record, uid);
        setEditingKidding(null);
        flash('success', 'Kidding record updated.');
      } else {
        await addRecord(orgId, collectionNames.goatKidding, record, uid);
        formElement.reset();
        flash('success', 'Kidding recorded.');
      }
    } catch (error) {
      flash('error', error instanceof Error ? error.message : 'Could not save kidding record.');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function submitMilk(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!animalId) return flash('error', 'Select a doe.');
    ...
    const transaction: Omit<Transaction, 'id'> | null =
      canFinance && revenue > 0
        ? { date, type: 'income', category: 'milk-sales', description: `Goat milk — ${animalLabel(animalId)}`, amount: revenue, method: 'cash' }
        : null;
    setBusy(true);
    try {
      if (editingMilk) {
        await updateRecordWithTransaction<typeof record>(orgId, collectionNames.goatMilkLogs, editingMilk.id, record, editingMilk.transactionId, transaction, canFinance, uid);
        setEditingMilk(null);
        flash('success', 'Milk record updated.');
      } else {
        await addRecordWithTransaction<Omit<MilkLog, 'id'>>(orgId, collectionNames.goatMilkLogs, record, transaction, uid);
        formElement.reset();
  ...
  async function submitWeight(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editingWeight) {
        await setRecord(orgId, collectionNames.goatWeights, editingWeight.id, record, uid);
        setEditingWeight(null);
        flash('success', 'Weight record updated.');
      } else {
        await addRecord(orgId, collectionNames.goatWeights, record, uid);
        formElement.reset();
  ...
  async function submitEvent(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (canFinance && cost > 0) {
      if (type === 'sold') {
        transaction = { date, type: 'income', category: 'goat-sales', description: `Goat sale — ${animalLabel(animalId)}`, amount: cost, method: 'cash' };
      } else if (type === 'vaccination' || type === 'treatment' || type === 'deworming') {
        transaction = { date, type: 'expense', category: type === 'vaccination' ? 'vaccine' : 'medication', description: `Goat ${type} — ${animalLabel(animalId)}`, amount: cost, method: 'cash' };
      }
    }
    ...
    try {
      if (editingEvent) {
        await updateRecordWithTransaction<typeof record>(orgId, collectionNames.goatEvents, editingEvent.id, record, editingEvent.transactionId, transaction, canFinance, uid);
        setEditingEvent(null);
        flash('success', 'Event updated.');
      } else {
        await addRecordWithTransaction<Omit<HerdEvent, 'id'>>(orgId, collectionNames.goatEvents, record, transaction, uid);
        formElement.reset();
        flash('success', 'Event recorded.');
      }
      if (type === 'sold' || type === 'dead') {
        await setRecord(orgId, collectionNames.goats, animalId, { status: type === 'sold' ? 'sold' : 'dead' }, uid);
      }
    } catch (error) {
  ...
  async function submitFeed(event: React.FormEvent<HTMLFormElement>) {
    ...
    const transaction: Omit<Transaction, 'id'> | null =
      canFinance && cost > 0
        ? { date, type: 'expense', category: 'feed', description: `Goat feed${animalIdRaw ? ` — ${animalLabel(animalIdRaw)}` : ' — herd'}`, amount: cost, method: 'cash' }
        : null;
    setBusy(true);
    try {
      if (editingFeed) {
        await updateRecordWithTransaction<typeof record>(orgId, collectionNames.goatFeedLogs, editingFeed.id, record, editingFeed.transactionId, transaction, canFinance, uid);
        setEditingFeed(null);
        flash('success', 'Feed record updated.');
      } else {
        await addRecordWithTransaction<Omit<HerdFeedLog, 'id'>>(orgId, collectionNames.goatFeedLogs, record, transaction, uid);
        formElement.reset();
  ...
  if (loading) {
    return <div className="rounded-3xl bg-white p-6 font-bold text-slate-600 shadow-card">Loading goat records...</div>;
  }
  ...
  return (
    <div>
      <PageHeader
        title="Goats"
        description="Herd register with kidding records, weight tracking, health events, and milk production for dairy breeds. Milk, sales, treatments, and feed post to finance automatically."
      />
      ...
      <Tabs<GoatsTab>
        tabs={[
          ...
          { value: 'kidding', label: 'Kidding Records' },
      ...
      {tab === 'overview' ? (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard label="Active goats" value={formatNumber(stats.active)} tone="forest" icon={<PawPrint size={18} />} />
          <StatCard label="Kids born (total)" value={formatNumber(stats.kidsBorn)} tone="clay" icon={<Baby size={18} />} />
          <StatCard label="Milk this month (L)" value={formatNumber(stats.milkThisMonth, 1)} tone="slate" icon={<Milk size={18} />} />
          ...
          {canWrite ? (
            <Card>
              ...
              <form onSubmit={submitAnimal} className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                <Input label="Ear tag / ID" value={animalForm.tag} onChange={e => setAnimalForm(f => ({ ...f, tag: e.target.value }))} placeholder="e.g. GT-001" />
                <Input label="Name" value={animalForm.name} onChange={e => setAnimalForm(f => ({ ...f, name: e.target.value }))} placeholder="Optional" />
                <Select label="Type" value={animalForm.type} onChange={e => setAnimalForm(f => ({ ...f, type: e.target.value as GoatType }))}
                  options={[{ label: 'Meat Goat', value: 'meat' }, { label: 'Dairy Goat', value: 'dairy' }, { label: 'Dual Purpose', value: 'dual' }]} />
                <Select label="Sex" value={animalForm.sex} onChange={e => setAnimalForm(f => ({ ...f, sex: e.target.value as GoatSex }))}
                  options={[{ label: 'Doe (female)', value: 'doe' }, { label: 'Buck (male)', value: 'buck' }, { label: 'Kid', value: 'kid' }]} />
                <Input label="Breed" value={animalForm.breed} onChange={e => setAnimalForm(f => ({ ...f, breed: e.target.value }))} placeholder="e.g. West African Dwarf" />
                ...
                {herd.map(a => {
                  ...
                  return (
                    <tr key={a.id}>
                      ...
                      <td className="px-4 py-3"><div className="flex gap-2">{editBtn(() => startEditAnimal(a))}{deleteBtn(collectionNames.goats, a.id, 'animal')}</div></td>
                    </tr>
                  );
                })}
              </Table>
            ) : <EmptyState title="No goats registered yet" />}
          </Card>
        </div>
      ) : null}
      ...
      {tab === 'kidding' ? (
        <div className="space-y-5">
          {canWrite ? (
            <Card>
              <CardTitle title={editingKidding ? 'Edit kidding record' : 'Record kidding'} action={editingKidding ? <Button variant="ghost" size="sm" onClick={() => setEditingKidding(null)}>Cancel edit</Button> : undefined} />
              <form key={editingKidding?.id ?? 'kid-new'} onSubmit={submitKidding} className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                <Select label="Doe" name="doeId" defaultValue={editingKidding?.doeId ?? ''} required>
                  <option value="">Select doe…</option>
                  {does.map(a => <option key={a.id} value={a.id}>{a.tag}{a.name ? ` — ${a.name}` : ''}</option>)}
                </Select>
                <Select label="Sire (buck)" name="buckId" defaultValue={editingKidding?.buckId ?? ''}>
                  <option value="">Unknown / N/A</option>
                  {bucks.map(a => <option key={a.id} value={a.id}>{a.tag}{a.name ? ` — ${a.name}` : ''}</option>)}
                </Select>
                <Input label="Date" name="date" type="date" defaultValue={editingKidding?.date ?? today()} />
                <Input label="Kids born" name="born" type="number" min={0} placeholder="0" defaultValue={editingKidding?.born ?? ''} />
                <Input label="Kids alive" name="alive" type="number" min={0} placeholder="0" defaultValue={editingKidding?.alive ?? ''} />
                <Input label="Gestation (days)" name="gestation" type="number" min={0} placeholder="e.g. 150" defaultValue={editingKidding?.gestation ?? ''} />
                <Textarea label="Notes" name="notes" placeholder="Optional" defaultValue={editingKidding?.notes ?? ''} />
                <div className="flex items-end"><Button type="submit" loading={busy} icon={<Baby size={16} />}>{editingKidding ? 'Save changes' : 'Record kidding'}</Button></div>
              </form>
            </Card>
          ) : null}

          <Card>
            <CardTitle title="Kidding records" />
            {kidding.length ? (
              <Table headers={['Date', 'Doe', 'Born', 'Alive', 'Actions']}>
                {kidding.map(k => (
                  <tr key={k.id}>
                    <td className="px-4 py-3">{dateLabel(k.date)}</td>
                    <td className="px-4 py-3 font-semibold">{animalLabel(k.doeId)}</td>
                    <td className="px-4 py-3">{formatNumber(k.born)}</td>
                    <td className="px-4 py-3">{formatNumber(k.alive)}</td>
                    <td className="px-4 py-3"><div className="flex gap-2">{editBtn(() => setEditingKidding(k))}{deleteBtn(collectionNames.goatKidding, k.id, 'kidding record')}</div></td>
                  </tr>
                ))}
              </Table>
            ) : <EmptyState title="No kidding records yet" />}
          </Card>
        </div>
      ) : null}
      ...
      {tab === 'milk' ? (
        <div className="space-y-5">
          {canWrite ? (
            <Card>
              ...
              <form key={editingMilk?.id ?? 'milk-new'} onSubmit={submitMilk} className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                <Select label="Doe" name="animalId" defaultValue={editingMilk?.animalId ?? ''} required>
                  <option value="">Select doe…</option>
                  {milkDoes.map(a => <option key={a.id} value={a.id}>{a.tag}{a.name ? ` — ${a.name}` : ''}</option>)}
                </Select>
          ...
          <Card>
            ...
            {milkLogs.length ? (
              <Table headers={['Date', 'Doe', 'Total (L)', 'Revenue', 'Actions']}>
                {milkLogs.map(l => (
                  <tr key={l.id}>
                    ...
                    <td className="px-4 py-3"><div className="flex gap-2">{editBtn(() => setEditingMilk(l))}{deleteBtn(collectionNames.goatMilkLogs, l.id, 'milk log')}</div></td>
                  </tr>
                ))}
              </Table>
              ...
              <Table headers={['Date', 'Animal', 'Weight (kg)', 'Notes', 'Actions']}>
                {weights.map(w => (
                  <tr key={w.id}>
                    ...
                    <td className="px-4 py-3"><div className="flex gap-2">{editBtn(() => setEditingWeight(w))}{deleteBtn(collectionNames.goatWeights, w.id, 'weight record')}</div></td>
                  </tr>
                ))}
              </Table>
              ...
              <Table headers={['Date', 'Animal', 'Type', 'Drug', 'Amount', 'Actions']}>
                {events.map(e => (
                  <tr key={e.id}>
                    ...
                    <td className="px-4 py-3"><div className="flex gap-2">{editBtn(() => setEditingEvent(e))}{deleteBtn(collectionNames.goatEvents, e.id, 'event')}</div></td>
                  </tr>
                ))}
              </Table>
      ...
      {tab === 'feed' ? (
        <div className="space-y-5">
          {canWrite ? (
            <Card>
              <CardTitle title={editingFeed ? 'Edit feed record' : 'Log goat feed'} description="Feed cost posts to finance." action={editingFeed ? <Button variant="ghost" size="sm" onClick={() => setEditingFeed(null)}>Cancel edit</Button> : undefined} />
              <form key={editingFeed?.id ?? 'feed-new'} onSubmit={submitFeed} className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                ...
                <Input label="Feed type" name="feedType" placeholder="e.g. Browse & concentrate" defaultValue={editingFeed?.feedType ?? ''} />
              ...
              <Table headers={['Date', 'Scope', 'Feed type', 'Qty (kg)', 'Cost', 'Actions']}>
                {feedLogs.map(log => (
                  <tr key={log.id}>
                    ...
                    <td className="px-4 py-3"><div className="flex gap-2">{editBtn(() => setEditingFeed(log))}{deleteBtn(collectionNames.goatFeedLogs, log.id, 'feed log')}</div></td>
                  </tr>
                ))}
              </Table>
